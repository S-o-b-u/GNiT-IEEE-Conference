...
import type { RegistrationFee, InsertRegistrationFee } from "@shared/schema";
...
export default function AdminFeesManager() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Fee Added", description: "Registration fee category has been created." });
      resetForm();
    },
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<RegistrationFee> }) =>
      apiRequest("PUT", `/api/registration-fees/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/registration-fees"] });
      toast({ title: "Fee Updated", description: "Registration fee has been updated." });
      resetForm();
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/registration-fees"] });
      toast({ title: "Fee Deleted", description: "Registration fee has been removed." });
    },
  });
    ...
    <AdminLayout>
      <div className="max-w-6xl">
        <div className="flex justify-between items-start mb-8">
          <div>
            <h1 className="text-3xl font-bold font-heading mb-2">Registration Fees</h1>
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
              <Button onClick={() => { resetForm(); setIsDialogOpen(true); }} data-testid="button-add-fee">
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{editingFee ? "Edit Fee" : "Add New Fee Category"}</DialogTitle>
                <DialogDescription>
                ...
                <div className="space-y-4 py-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setFormData(prev => ({ ...prev, indianFee: e.target.value }))}
                      placeholder="₹10,000"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="internationalFee">International Delegate Fee *</Label>
                    <Input
                      id="internationalFee"
                      value={formData.internationalFee}
                      onChange={(e) => setFormData(prev => ({ ...prev, internationalFee: e.target.value }))}
                      placeholder="$200"
          ...
          <CardContent>
            {isLoading ? (
              ...
              <p className="text-muted-foreground text-center py-8">
                No fee categories added yet. Click "Add Fee Category" to create one.
              </p>
            )}
          </CardContent>
        </Card>
